...
class PctEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # B, D, N
        # x:32,3,1024
        batch_size, point_dim, N = x.size()
        assert point_dim == 3, f'point_dim must be 3, wrong pcl shape {x.size()}'
        ...
        feature = feature_0.permute(0, 2, 1)  # feature:[32, 128, 512]
        new_xyz, new_feature = sample_and_group(npoint=256, radius=0.2, nsample=32, xyz=new_xyz, points=feature)
        feature_1 = self.gather_local_1(new_feature)

        x = self.pt_last(feature_1)  # x:[32,1024,256]
        x = torch.cat([x, feature_1], dim=1)  # x:[32,1280,256]
